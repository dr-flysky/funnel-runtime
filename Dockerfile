...
# its own config instead — the service volume on Railway, disk.mountPath on
# Render, `-v` for a local `docker run`. When nothing is mounted, openDb()
# just creates /data as an ordinary directory.
